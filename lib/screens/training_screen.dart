...
class _TrainingScreenState extends State<TrainingScreen> {
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      // Platform-specific image format configuration
      ...
      if (mounted && _cameraController!.value.isInitialized) {
        _cameraController!.startImageStream(_processCameraImage);
        print('✅ Camera initialized successfully for ${Platform.operatingSystem}');
      }
    } catch (e) {
      print('❌ Error initializing camera: $e');
  ...
  Future<void> _initializePoseDetection() async {
    try {
      final success = await _poseService.initializePoseDetection();
      if (!success) {
        throw Exception('Failed to initialize pose detection');
      }
    } catch (e) {
